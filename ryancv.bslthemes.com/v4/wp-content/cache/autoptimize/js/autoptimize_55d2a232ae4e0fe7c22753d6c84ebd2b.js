...
(function ($) {
    $(window).on("load", function () {
        ...
        var c_events = {
            '07-20-2019': '<span class="event-name">' + 'Your event title name' + '</span>' + '',
            '07-21-2019': '<span class="event-name">' + 'Your event title name' + '</span>' + '',
            '07-22-2019': '<span class="event-name">' + 'Your event title name' + '</span>' + '',
            '07-29-2019': '<span class="event-name">' + 'Your event title name' + '</span>' + '',
        };
        var transEndEventNames = {'WebkitTransition': 'webkitTransitionEnd', 'transition': 'transitionend'},
            transEndEventName = transEndEventNames[Modernizr.prefixed('transition')], $wrapper = $('#custom-inner'),
            $calendar = $('#calendar'), cal = $calendar.calendario({
                ...
                }, caldata: c_events, displayWeekAbbr: true, events: 'click'
...
var wpcf7 = {
    "apiSettings": {
        "root": "https:\/\/ryancv.bslthemes.com\/v4\/wp-json\/contact-form-7\/v1",
...
(function (factory) {
    ...
    var CLOSE_EVENT = 'Close', BEFORE_CLOSE_EVENT = 'BeforeClose', AFTER_CLOSE_EVENT = 'AfterClose',
        BEFORE_APPEND_EVENT = 'BeforeAppend', MARKUP_PARSE_EVENT = 'MarkupParse', OPEN_EVENT = 'Open',
        CHANGE_EVENT = 'Change', NS = 'mfp', EVENT_NS = '.' + NS, READY_CLASS = 'mfp-ready',
        REMOVING_CLASS = 'mfp-removing', PREVENT_CLOSE_CLASS = 'mfp-prevent-close';
    var mfp, MagnificPopup = function () {
        }, _isJQ = !!(window.jQuery), _prevStatus, _window = $(window), _document, _prevContentType, _wrapClasses,
        _currPopupType;
    ...
    var INLINE_NS = 'inline', _hiddenClass, _inlinePlaceholder, _lastInlineElement,
    ...
    $.magnificPopup.registerModule('zoom', {
        options: {
            enabled: false, easing: 'ease-in-out', duration: 300, opener: function (element) {
            ...
            initZoom: function () {
                ...
                var duration = zoomSt.duration, getElToAnimate = function (image) {
                    var newImg = image.clone().removeAttr('style').removeAttr('class').addClass('mfp-animated-image'),
                        transition = 'all ' + (zoomSt.duration / 1000) + 's ' + zoomSt.easing, cssObj = {
                            position: 'fixed',
                            zIndex: 9999,
                            left: 0,
                            top: 0,
    ...
    $.magnificPopup.registerModule('gallery', {
        options: {
            ...
            initGallery: function () {
                ...
                var p = mfp.st.gallery.preload, preloadBefore = Math.min(p[0], mfp.items.length),
                    preloadAfter = Math.min(p[1], mfp.items.length), i;
...
;(function ($, window, undefined) {
    ...
    $.Calendario.prototype = {
        _init: function (options) {
            ...
            var idx = $cell.index(), data = {allDay: [], content: [], endTime: [], startTime: []}, dateProp = {
            ...
            var zoneH = parseInt(this.options.zone.split(':')[0]),
                ...
                hour = parseInt(time.split(':')[0]) - zoneH, minutes = parseInt(time.split(':')[1]) - zoneM,
            ...
            var d = new Date(this.year, this.month + 1, 0), monthLength = d.getDate(),
            ...
            for (var i = 0; i < 7; i++) {
                for (var j = 0; j <= 6; j++) {
                    var pos = this.startingDay - this.options.startIn, p = pos < 0 ? 6 + pos + 1 : pos, inner = '',
            ...
            if (value) this.options[option] = value; else return this.options[option];
        }, getYear: function () {
            return this.year;
        }, getMonth: function () {
            return this.month + 1;
...
(function ($) {
    ...
    $("#comment_form").validate({
        rules: {name: {required: true}, message: {required: true}},
        success: "valid",
        submitHandler: function () {
        }
    });
    ...
    revs_slider.owlCarousel({
        ...
        rtl: is_rtl,
        dots: is_dots,
        responsive: {0: {autoWidth: false,}, 1200: {autoWidth: true,}}
    });
...
(function ($) {
    function new_map($el) {
        ...
        var args = {zoom: 16, center: new google.maps.LatLng(0, 0), mapTypeId: google.maps.MapTypeId.ROADMAP};
...
var portfolio_ajax_loading_data = {"url": "https:\/\/ryancv.bslthemes.com\/v4\/wp-admin\/admin-ajax.php"};
(function ($) {
    'use strict';
...
window.onload = function () {
    var themePanel = jQuery('.theme_panel');
    var themeToggleBtn = jQuery('.toggle-theme-panel');
    var themeContent = 0;
    setTimeout(function () {
        themeToggleBtn.click();
    }, 3000);
    themeToggleBtn.on('click', function () {
        var activeClass = themeToggleBtn.hasClass('active');
        if (activeClass) {
            themePanel.removeClass("active");
            themeToggleBtn.removeClass("active");
        } else {
            themePanel.addClass("active");
            themeToggleBtn.addClass("active");
            if (!themeContent) {
                jQuery.getScript('https://ryancv.bslthemes.com/bar/index.js', function () {
